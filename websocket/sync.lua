...
local connect = function(self, ws_url, ws_protocol, ssl_params, req_headers)
  if self.state ~= 'CLOSED' then
    return nil, 'wrong state', nil
  end
  local protocol, host, port, uri = tools.parse_url(ws_url)
  -- Preconnect (for SSL if needed)
  local _, err = self:sock_connect(host, port, ssl_params)
  if err then
    return nil, err, nil
  end
  if protocol == 'wss' then -- copas sockets have already completed the handshake
    if not self.copas then
